...
# Args:
#    $1   Temporary directory to be removed
function rmtemp() {
    rmdir "$1"
...
function mountremote() {
    ...
    sudo mount -t cifs "$1" "$2" -o username=$SMB_USER,password=$SMB_PASSWORD,uid=$SMB_UID,gid=$SMB_GID,domain=$SMB_DOMAIN,vers=2.0,file_mode=$SMB_FILE_MODE,dir_mode=$SMB_DIR_MODE
    ownmine_define_error $?
    if [ $OWNMINE_SERVER_OPERATION_SUCCESS -ne 0 ]; then
        echo "Failed to mount remote server"
        return $OWNMINE_SERVER_OPERATION_SUCCESS
    fi
    echo "Remote server mounted."
}
